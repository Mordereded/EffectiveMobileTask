...
namespace Project_test_task
{
    ...
    public partial class MainMenu : Form
    {
        ...
        private void button9_Click_1(object sender, EventArgs e)
        {
            ...
            tests.RunAllTests();*/
            string assemblyPath = @"..\..\..\..\Testing_project\bin\Debug\net8.0-windows\Testing_project.dll";
            Assembly testAssembly = Assembly.LoadFrom(assemblyPath);

            Type testClassType = testAssembly.GetType("Tests.UnitTests");
            if (testClassType == null)
            {
                logger.Log("Класс с именем 'Namespace.ClassName' не найден.");
                return;
            }
            object classInstance = Activator.CreateInstance(testClassType);
            MethodInfo runTestsMethod = testClassType.GetMethod("RunTests");
            if (runTestsMethod == null)
            {
                logger.Log("Метод 'RunTests' не найден.");
                return;
            }
            var result = runTestsMethod.Invoke(classInstance, null) as StringBuilder;
            if (result != null)
            {
            ...
            {
                logger.Log("Метод 'RunTests' не вернул результат.");
            }
